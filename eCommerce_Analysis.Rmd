...
#Next, let's exclude the transaction involving SKU not present in the datafile "Categories_all.csv"
blacklist=NULL
for (i in 1:nrow(Transaction_data)){
  if (!(as.character(Transaction_data[i,"SKU"]) %in% names(SKU_Cat))){
    blacklist=c(blacklist,i)
  }
}

#we keep only the ones that are present in the datafile "Categories_all.csv"
Transaction_data<-Transaction_data[-blacklist,]
...
qplot(Number_of_Items_Purchased, 
      ...
      geom="histogram",
      binwidth=1) + scale_x_continuous(limits = c(0, 15))
...
qplot(Number_of_Items_Purchased, 
      data=Items_sold_per_customer, 
      geom="histogram",
      binwidth=1) + scale_x_continuous(limits = c(0, 15)) + scale_y_continuous(limits = c(0, 1000))
...
Items_sold_per_customer_restricted = as.data.frame(apply(Matrix_segm,1,sum))
names(Items_sold_per_customer_category)<-"Number_of_Items_Purchased"

qplot(Number_of_Items_Purchased, 
      data=Items_sold_per_customer_category, 
      geom="histogram",
      binwidth=1) + scale_x_continuous(limits = c(0, 15))
...
ggplot(x=rownames(Items_sold_per_category),
      y= apply(Category_matrix,2,sum), 
      geom="hist") 
