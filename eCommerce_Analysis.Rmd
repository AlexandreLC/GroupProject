...
Sales_Cust_Items = matrix(0*1:(nbCustomers*nbItemsSold), nrow=nbItemsSold, ncol=nbCustomers)
...
for (i in 1:nbTransactions){
  ...
  Sales_Cust_Items[itemsold, customer]<-Sales_Cust_Items[itemsold, customer]+1
}
...
Single_Customer_JnF=unique(Customer_JnF[1])
...
for (i in 1:ncol(Sales_Cust_Items)){
  if ((sum(Sales_Cust_Items[,i])>=Item_threshold)&!(as.character(colnames(Sales_Cust_Items)[i]) %in% Single_Customer_JnF)){
    Customer_segm=c(Customer_segm, as.character(colnames(Sales_Cust_Items)[i]))
    Item_to_add=names(Sales_Cust_Items[Sales_Cust_Items[,i]>0,i])
...
# The rows of the matrix will be the items bought and the column will be the customers

Matrix_segm=matrix(0*1:(length(Items_segm)*length(Customer_segm)), nrow = length(Items_segm), ncol = length(Customer_segm))

rownames(Matrix_segm)<-Items_segm
colnames(Matrix_segm)<-Customer_segm

Matrix_segm[Items_segm,Customer_segm]<-Sales_Cust_Items[Items_segm,Customer_segm]
...
```{r, echo=FALSE}
#The goal of this section is to do a segmentation of the data from the second dataset
