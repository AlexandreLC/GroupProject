...
Orders_per_customer = matrix(0*1:(nbCustomers*nbOrders), ncol=nbOrders, nrow=nbCustomers)
colnames(Orders_per_customer) <- Orders_list
rownames(Orders_per_customer) <- Customer_list

for (i in 1:nbTransactions){
  order=as.character(Transaction_data[i,"Order.Number"])
  customer=as.character(Transaction_data[i,"Anonym"])
  Orders_per_customer[customer,order]<-Orders_per_customer[customer,order]+1
}
...
Orders_per_customer = count(Transaction_data,"Anonym")

ggplot(Orders_per_customer, aes(Orders_per_customer$freq)) + geom_histogram(fill = '#B8274C', binwidth = 0.5) + scale_x_continuous(breaks = seq(0,15), limits = c(0, 15)) + labs(x = "Number of orders")
...
Orders_per_customer[Orders_per_customer > 1] <- 1

Ordered_by_customer = as.data.frame(apply(Orders_per_customer,1,sum))
names(Ordered_by_customer)<-"Number_of_Orders"

ggplot(Ordered_by_customer, aes(Ordered_by_customer$Number_of_Orders)) + geom_histogram(fill = '#B8274C', binwidth = 0.5) + scale_x_continuous(breaks = seq(0,15), limits = c(0, 15)) + labs(x = "Number of orders")
